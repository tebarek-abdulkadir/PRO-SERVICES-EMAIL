     ...
     * Same logic as n8n-chat-analysis-email-code.js, but the report “today” is the calendar day *before*
     * the current day in REPORT_TZ (e.g. run on Apr 28 → reportDate Apr 27, MTD = 1st of that month through Apr 27).
     ...
     * Operations: .../operations/{YYYY-MM-DD}.json — same report calendar day as the rest of this email (yesterday-as-today anchor).
     *
    ...
    function getServiceKeyFromComplaintType(complaintType) {
      ...
      if (normalized.includes('schengen')) {
        ...
        if (schengenDest.some((c) => normalized.includes(c))) return 'schengen';
        return undefined;
      }
    ...
    function overviewLabelForServiceKey(sk) {
      switch (sk) {
        case 'oec':
          return 'OEC';
        case 'owwa':
    ...
    function aggregateComplaintSummaryByOverviewLabel(summary) {
      const map = new Map();
      for (const raw of summary) {
        const ct = String(raw.COMPLAINT_TYPE ?? raw.complaint_type ?? '').trim();
        if (!ct) continue;
        const sk = getServiceKeyFromComplaintType(ct);
        if (!sk) continue;
        const label = overviewLabelForServiceKey(sk);
        if (!label) continue;
        const y = Number(raw.YESTERDAY) || 0;
        const m = Number(raw.THIS_MONTH) || 0;
        const l = Number(raw.LAST_MONTH) || 0;
    ...
    function applyComplaintSummaryTotalsToRows(rows, summary) {
      const byLabel = summary && summary.length ? aggregateComplaintSummaryByOverviewLabel(summary) : null;
      return rows.map((r) => {
        const t = byLabel && byLabel.get(r.label);
        return {
          ...r,
          totalSalesYesterday: t ? t.yesterday : 0,
          totalSalesThisMonth: t ? t.thisMonth : 0,
          totalSalesLastMonth: t ? t.lastMonth : 0,
        };
      });
    }
    ...
    const SCHENGEN_ALLOWED_COUNTRY_ALIAS_TOKENS = [
      'france',
      'germany',
      'spain',
      'switzerland',
    ...
    const SCHENGEN_REGION_ALIASES = ['schengen', ...SCHENGEN_ALLOWED_COUNTRY_ALIAS_TOKENS];
    ...
    /** Resolution order: Lebanon → Egypt → Jordan → Saudi → Schengen (listed destinations only). */
    function resolveEmailTravelRegionKey(travelVisaCountries) {
      ...
      for (const { key, aliases } of EMAIL_TRAVEL_REGIONS) {
        if (prospectCountriesMatchRegions(travelVisaCountries, aliases)) return key;
      }
      return null;
    }
    ...
    function buildServiceOverviewRows(byContractType, countryCountsByContractType, emailSalesCcMv) {
      ...
      return [
        ...
        row('Visa Saudi', pSaudiCc, pSaudiMv, visa('Visa Saudi').cc, visa('Visa Saudi').mv),
    ...
    function computeExtendedTotalsRow(rows, mtdSnapshots, lmSnapshots) {
      ...
      for (const r of rows) {
        ...
        totalSalesYesterday += r.totalSalesYesterday || 0;
        totalSalesThisMonth += r.totalSalesThisMonth || 0;
        totalSalesLastMonth += r.totalSalesLastMonth || 0;
      }
    ...
    async function loadMergedComplaintsFromBlobRange(startIso, endIso) {
      ...
      for (const d of dates) {
        ...
        const list = j && Array.isArray(j.complaints) ? j.complaints : [];
    ...
    function serviceOverviewDataCells(row, bold = false) {
      ...
      return `
              ...
              <td style="${s}">${row.totalSalesYesterday ?? 0}</td>
              <td style="${s}">${row.totalSalesThisMonth ?? 0}</td>
              <td style="${s}">${row.totalSalesLastMonth ?? 0}</td>`;
    }
    ...
    function renderServiceOverviewTable(rows, totals, periodNote, colToday) {
      ...
      const note = periodNote
        ? `<div style="font-size:10px;color:#757575;margin:8px 0 0 0;line-height:1.35;">MTD totals sum days with valid <strong>daily/{date}.json</strong> (${periodNote.mtdDaysCounted} day(s); missing days skipped for all rows). LM uses the prior calendar month (${periodNote.lmDaysCounted} day(s)). Prospects exclude any contract/maid/client with a complaint <em>before</em> that day (from merged <strong>complaints-daily/*.json</strong> in a ${COMPLAINT_BLOB_LOOKBACK_DAYS}-day lookback). <strong>Sales out of tracked prospects</strong> match the dashboard: complaint types on that day vs filtered prospects. <strong>Total sales</strong> columns use <code>complaints-daily/{report-date}.json</code> <code>summary</code> (YESTERDAY / THIS_MONTH / LAST_MONTH).</div>`
        : '';

      const lmLabel = 'Last month';
      return `
        <table role="presentation" width="100%" style="border:1px solid #bdc3c7;border-collapse:collapse;width:100%;min-width:1080px;margin:0 0 24px 0;mso-table-lspace:0pt;mso-table-rspace:0pt;table-layout:fixed;" cellspacing="0" cellpadding="0">
          <thead>
            <tr>
              <th style="${thLeft}" rowspan="3">Service Type</th>
              <th style="${thStyle}" colspan="6">Prospect</th>
              <th style="${thStyle}" colspan="6">Sales out of tracked prospects</th>
    ...
    async function buildReport() {
      const calendarTodayInTz = getDateInTimeZone(new Date(), REPORT_TZ);
      const reportDate = addCalendarDays(calendarTodayInTz, -1);
      const displayDate = formatDisplayDate(reportDate, REPORT_TZ);
      const colToday = shortDateColumnLabel(reportDate, REPORT_TZ);
      ...
      let section1Html = '';
      if (todaySvc) {
        const complaintDayDoc = await fetchComplaintsBlob.call(this, reportDate);
        const summary =
          complaintDayDoc && Array.isArray(complaintDayDoc.summary) ? complaintDayDoc.summary : null;
        const agg = applyPeriodAggregatesToRows(todaySvc, mtdSnapshotsSvc, lmSnapshotsSvc);
        const rowsWithTotals = applyComplaintSummaryTotalsToRows(agg.rows, summary);
        const totals = computeExtendedTotalsRow(rowsWithTotals, mtdSnapshotsSvc, lmSnapshotsSvc);
        section1Html =
          sectionTitle('1', 'Service Overview') +
          renderServiceOverviewTable(rowsWithTotals, totals, { mtdDaysCounted: agg.mtdDaysCounted, lmDaysCounted: agg.lmDaysCounted }, colToday);
      } else {
        section1Html =
          sectionTitle('1', 'Service Overview') +
      ...
      const text = [
        ...
        '3. Operations Summary — from operations/{report-date}.json (see HTML).',
        '',
